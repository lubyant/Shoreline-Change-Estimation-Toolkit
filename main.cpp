...
void func(vector<string> &paths) {
    ...
    for (const auto &path: paths) {
        ...
        auto shores =
                im::extract_shorelines(contour, image.rows - 1, image.cols - 1, 2000);
        shores_years.push_back(shores);
    }
    auto &&baseline =
            im::create_baseline(shores_years[0], transect_length, spacing, 0, 50);
    auto intersections = im::create_intersections(baseline, shores_years);
}