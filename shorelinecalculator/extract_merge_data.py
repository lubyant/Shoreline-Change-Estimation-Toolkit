...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 19 12:28:10 2022
@author: bbean
"""
import shutil
import re
...
# This function returns all files within a folder with a certain suffix
...
# This function saves a variable into a pickle file
...
# This function extracts the metadata within a raster file
...
# This function update meta data for 1 channel image from 3 channel image
...
# This function save an img with metadata into a raster tif 
...
# This function returns the img saved in a tif file
def get_img_from_raster(raster_folder, raster_file): 
    ...
    res = np.stack((np.uint8(myarray3), np.uint8(myarray2),np.uint8(myarray1)), axis = 2)
    return res
...
# This function returns the infrared img saved in a tif file
def get_infrared_img_from_raster(raster_folder, raster_file):    
    ds = gdal.Open(raster_folder + raster_file)
    myarray1 = np.array(ds.GetRasterBand(4).ReadAsArray())
    return myarray1

# This function divides an img into many sub-imgs with the size of row_interval and col_interval
def divide_img(img, out_folder, out_name, row_interval = 1000, col_interval = 1000):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if len(img.shape) > 2:
                cv2.imwrite(out_folder + img_name, img[r_start : r_end, c_start : c_end, :])
            else:
                cv2.imwrite(out_folder + img_name, img[r_start : r_end, c_start : c_end])
...
# This function returns the row and col division number of a set of splitted file names
...
# This function searchs if a folder contains '_1_' and '_h_'
...
# This function finds the file that contains certain labels
...
# This function merges the imgs for a sample id within a year of a direction
def merge_img(in_folder, sample_id, year, direction):
    ...
    for i in range(max_r + 1):
        ...
        for j in range(max_c + 1):
            ...
            temp_img = cv2.imread(check_folder + '%d_%d_%s'%(i, j, suffix_name))
            

            if j == 0:
                temp_r, temp_g, temp_b = temp_img[:, :, 0], temp_img[:, :, 1], temp_img[:, :, 2]
            else:
...
# This function gets the metadata for the splitted img
...
# This function returns the metadata file names of a sample id within a year
...
# This function returns the metadata file name for a direction
...
def get_merged_data_by_point(img_folder, pckl_folder, raster_save_folder, sample_id, year):
    ...
    for direction in dir_list:
        ...
        if not temp_meta_name:
            ...
            saved_img = np.stack((temp_img[:,:,2], temp_img[:,:,1], temp_img[:,:,0]), axis = 0)
            temp_meta.update({
                "driver": "GTiff",
                "height": temp_img.shape[0],
                "width": temp_img.shape[1],
            })
            save_new_raster(saved_img, temp_meta, temp_save_folder, raster_name)
    ...
    if file_tifs:
        ...
        if len(file_tifs) == 1:
            shutil.copy(temp_save_folder + file_tifs[0], final_save_folder + final_save_name)
        else:
            with rasterio.open(temp_save_folder + file_tifs[0]) as src:
                meta = src.meta.copy()
            print("merge start")
            to_merge = [rasterio.open(temp_save_folder + f0) for f0 in file_tifs]
