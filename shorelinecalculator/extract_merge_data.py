...
def divide_img(img, out_folder, out_name,
               row_interval=1000, col_interval=1000):
...
def divide_img_by_interval(img, out_folder, out_name,
                           row_interval=1000, col_interval=1000):
    check_folder_exist(out_folder)
    row_num, col_num = img.shape[:2]
    rows, cols = row_num // row_interval, col_num // col_interval
...
def divide_img_by_interval_using_pil(channel1, channel2, channel3,
                                     out_folder, out_name,
                                     row_interval=1000, col_interval=1000):
    check_folder_exist(out_folder)
...
def divide_grayscale_img_by_interval_using_pil(channel1, out_folder, out_name,
                                               row_interval=1000,
                                               col_interval=1000):
    check_folder_exist(out_folder)
    row_num, col_num = channel1.shape[:2]
...
def merge_img(in_folder, year, direction):
    ...
    for i in range(max_r + 1):
        ...
        for j in range(max_c + 1):
            ...
            if j == 0:
                temp_r, temp_g, temp_b = temp_img[:, :, 0], \
...
def get_split_meta(out_meta, r_start, r_end, c_start, c_end):
    ...
    out_meta.update({
        ...
        "height": row_num,
        "width": col_num,
        "transform": Affine(gdal_trans[1], gdal_trans[2], \
                            Xgeo, gdal_trans[4], \
                            gdal_trans[5], Ygeo)
    })
    return out_meta
...
def get_merged_data_by_point(img_folder, pckl_folder,
                             raster_save_folder, sample_id, year):
    ...
    for direction in dir_list:
        ...
        if not temp_meta_name:
            ...
            saved_img = np.stack(
                (temp_img[:, :, 2], temp_img[:, :, 1], temp_img[:, :, 0]),
                axis=0)
            temp_meta.update({
                "driver": "GTiff",
    ...
    if file_tifs:
        ...
        if len(file_tifs) == 1:
            ...
            # The merge function returns a single array and
            # the affine transform info
            ...
            with rasterio.open(final_save_folder + final_save_name,
                               "w", **meta) as dest:
                dest.write(arr)
    ...
    for z in zip_files:
        ...
        try:
            ...
            divide_img_by_interval(
                orig_img, img_orig_folder, file_front + '.' + 'png',
                row_interval=1000, col_interval=1000)
            if raster_file.split('_')[0] == 'm' or\
                    raster_file.split('_')[0] == 'M':
                infrared_img = get_infrared_img_from_raster(
                    raster_save_folder, raster_file)
                divide_grayscale_img_by_interval_using_pil(
                    infrared_img, infrared_orig_folder, file_front + '.png',
                    row_interval=1000, col_interval=1000)
...
if __name__ == '__main__':
    ...
    zip_folder, raster_folder, txt_folder, img_folder, infrared_folder, pckl_folder = zip_folder + lake + '/', raster_folder + lake + '/', \
        txt_folder + lake + '/', img_folder + lake + \
        '/', infrared_folder + lake + '/', pckl_folder + lake + '/'
    ...
    if stage == 2:
        ...
        get_merged_data_by_point(
            img_orig_folder, pckl_orig_folder, final_save_folder, '4108603', '2018')
